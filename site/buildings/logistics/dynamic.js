...
void main(){vec4 map=texture(uMap,vUV);if(map.a<.5){finalColor=vec4(0.);return;}
 ...
 float marks=clamp(max(glyph,pat)*.9,0.,1.);
  ...
  float edge=max(uFillBounds.z,.0001);float occupied=0.;
  if(uFilled>.5&&fm.a>.5){occupied=smoothstep(uFillBounds.x-edge,uFillBounds.x+edge,fluidS)*(1.-smoothstep(uFillBounds.y-edge,uFillBounds.y+edge,fluidS));}
  float shimmer=0.;
  if(occupied>0.){vec2 normal=src(uFlow,vec2(fm.r*2.,fluidS-uTime*uWidths.z)).rg*2.-1.;shimmer=pow(abs(normal.x*.7+normal.y*.3),2.)*.20*occupied;}
  if(uLayer<.5){alpha=marks;rgb=vec3(.72,.91,1.);}
  else if(uLayer<1.5){alpha=shimmer;rgb=vec3(.75,.88,1.);}
  else{vec4 base=texture(uBase,vUV);if(uLayer<2.5)base.rgb*=uTint.rgb;finalColor=base*occupied*uColor;return;}
 }
...
export async function loadDynamic(PIXI,collection,signal){
 ...
 const cycle={fillCellsPerSecond:pipeParams.fillCellsPerSecond??8,drainCellsPerSecond:pipeParams.drainCellsPerSecond??8,edgeWidth:pipeParams.fillEdgeWidth??.04};
