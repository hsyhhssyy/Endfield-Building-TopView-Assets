...
const vertex='attribute vec2 aPosition;attribute vec2 aUV;uniform float uZoom;varying vec2 vUV;void main(){vUV=aUV;gl_Position=vec4(aPosition.x*uZoom/7.5,-aPosition.y*uZoom/7.5,0.,1.);}';
...
// Preview grid and mesh use the same 15-unit viewport and zoom.
const grid=$('grid'), gridContext=grid.getContext('2d');
let gridKey='';
function drawGrid(size,zoom){
 const enabled=$('show-grid').checked,key=[size,zoom,enabled,$('background').value].join(':');
 if(key===gridKey)return;gridKey=key;grid.width=grid.height=size;
 grid.hidden=!enabled;if(!enabled)return;
 const ctx=gridContext,scale=size*zoom/15,point=x=>size/2+x*scale;
 const light=$('background').value==='#ffffff';
 ctx.lineWidth=Math.max(1,size/800);ctx.strokeStyle=light?'rgba(25,45,35,.18)':'rgba(210,230,220,.16)';
 ctx.beginPath();
 for(let i=1;i<13;i++){const p=point(-6.5+i);ctx.moveTo(p,point(-6.5));ctx.lineTo(p,point(6.5));ctx.moveTo(point(-6.5),p);ctx.lineTo(point(6.5),p);}
 ctx.stroke();ctx.strokeStyle=light?'rgba(25,45,35,.45)':'rgba(210,230,220,.42)';ctx.setLineDash([4,5]);
 ctx.strokeRect(point(-6.5),point(-6.5),13*scale,13*scale);ctx.setLineDash([]);
}
...
$('seek').oninput=()=>{time=Number($('seek').value);running=false;$('play').textContent='播放';};
